...
namespace SIGDAI.MVC.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ExternalLoginCallback(string provider, string returnUrl)
        {
            ...
            vUsuarioActivo = Request.LogonUserIdentity.Name;            
            ...
            if (vUsuario != null)
            {
                ApplicationUser vRegistrado = UserManager.FindById(vUsuario.IdUsuario.ToString());
                if (vRegistrado == null)
                {                    
                    var user = new ApplicationUser { UserName = vUsuarioActivo, Email = vUsuario.Correo, Id = vUsuario.IdUsuario.ToString()};
                    var vresult = await UserManager.CreateAsync(user);
                    if (vresult.Succeeded)
                    {
                        vresult = await UserManager.AddLoginAsync(user.Id, loginInfo.Login);
                        if (vresult.Succeeded)
                        {
                            await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);                            
                        }
